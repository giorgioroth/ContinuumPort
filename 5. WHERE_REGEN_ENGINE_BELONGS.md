...
(part of a broader suite of 1976 tests, passing in roughly 6-8s across runs)
```
...
- commit-boundary perimeter (structurally enumerated: exactly one component in the kernel implements a commit boundary, verified by static analysis rather than by name)
