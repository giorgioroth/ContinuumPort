...
> **Status: non-normative.** This document states field-of-use reasoning and positioning. It is not part of the ContinuumPort standard, not a specification, and not a grant of rights. The normative documents are the CP-Core specification and the contents of `normative/`; the governing terms are in [`3. LICENSE_REGEN.md`](https://github.com/giorgioroth/ContinuumPort/blob/main/3.%20LICENSE_REGEN.md). Where this document and the license differ, the license governs.
...
**Complete mediation, and what it does not yield.** Saltzer and Schroeder (1975) require that every access to every object be checked for authority, and state that systematic application of the principle forces a system-wide view of access control. The requirement that no effect-capable path reach the effect unmediated is that principle. Two things follow that this document previously ran together. First, complete mediation forbids *unmediated* paths; it does not forbid *duplicate* mediators, and two correct enforcement points satisfy it equally. Second, the same authors place inside complete mediation an obligation this project met late: proposals to gain performance by remembering the result of an authority check are to be treated sceptically, and when authority changes, remembered results must be systematically updated. An authorization carried across a propose/commit boundary is such a remembered result.
...
- control–effect binding (the subject validated for admissibility is the same object whose effects commit; a caller-supplied description of intent cannot change the verdict on a canonical transition)
- fail-closed ordering (for an inadmissible transition the actuator is never invoked, not invoked-then-rolled-back; an admissible one still reaches it — the positive control)
- audit provenance (the recorded effect is derived from the committed transition, not from the caller's description, and the two fields can disagree)
- commit-boundary perimeter (the kernel is enumerated structurally, by static analysis rather than by name, and the test asserts that exactly one component implements a commit boundary)
...
**Where the perimeter figure stands, and where it does not.** That exactly one commit boundary exists is a REGEN architectural constraint, enforced by the test above and **not ratified in a normative document**. It does not follow from any principle cited in this document. Complete mediation forbids unmediated paths, not duplicate mediators; economy of mechanism motivates minimising mechanism, and its authors state that their principles serve as warnings rather than absolute rules. A warning to minimise fixes no cardinality. The test verifies the constraint; it does not derive it.

**Provenance of the last four entries.** The control–effect binding, fail-closed ordering, audit provenance and commit-boundary perimeter tests were written on 13 August 2026 as repairs, not as pre-existing coverage. They exist because an audit of this kernel found the commit boundary defective: the effect was materialised before the check ran, the check took its subject from an argument supplied separately from the transition, the validation loop terminated after its first element, an empty argument skipped it entirely, and a second executable implementation of the same boundary existed in the tree. The suite in force at the time reported no failures throughout. That episode, the earlier public claim it contradicts, and what the evidence package failed to establish are written up separately rather than summarised here. Listing the instruments without their origin would present as coverage what is in fact a repair.

The control–effect binding property is written up as an internal normative document (status: proposed, pending ratification) rather than asserted only through test names — the same discipline this document applies to its own claims: a guarantee is stated, its counterexamples are listed, and it is marked closed only once every clause has an instrument.
...
**License gate — a separate restriction from the evidence question.** These are also the domains the Regen Engine license places outside its permitted field of use, and that restriction does not lift when the evidence arrives. Independent validation would not by itself authorize deployment here. Under the current Beta license, any use in medical devices, clinical or neural-interface systems, autonomous vehicles, aviation, launch or propulsion control, industrial safety systems, or any system in which a malfunction could cause death, personal injury, or significant property or environmental harm requires **both** a separate written agreement with the author **and** independent safety qualification and regulatory conformity assessment performed by the integrator. Absent both, such use is unauthorized. The two conditions are cumulative, not alternatives. See the Safety & Field-of-Use Notice in [`3. LICENSE_REGEN.md`](https://github.com/giorgioroth/ContinuumPort/blob/main/3.%20LICENSE_REGEN.md).
...
*Argued, not demonstrated: a hypothetical rejection would look like `[REGEN] REJECTED (geometry) → velocity 999.0 exceeds declared limit 130.0` — but no such rejection has been produced on real vehicle hardware, and this is not a log entry from an actual run.*
...
**Precondition — read with the claim above.** This guarantee holds for any adapter that implements the compliance interface correctly. An adapter that does not implement it, or that bypasses it, is outside this guarantee entirely — there is no enforcement over code that never enters through the kernel. Enforcement applies to compliant adapters, not to arbitrary code. The trust assumptions this contract relies on are documented in [`EXECUTION_MODEL_LIMITS.md`](https://github.com/giorgioroth/ContinuumPort/blob/main/docs/EXECUTION_MODEL_LIMITS.md) §2.9.
...
**How this list is warranted, and how it is not.** Each item above is a property the kernel is built to hold and the suite is built to exercise. Neither the building nor the exercising establishes that the property holds: a test establishes a property only relative to a demonstrated failure mode under which violating the property makes the test fail. Where that demonstration has been performed — by reintroducing the defect and confirming the suite goes red — it is internal evidence of the suite's sensitivity, not independent reproduction. A reader cannot run the suite, and the correct reading of every item above is "this is claimed and instrumented," not "this is verified for you."
...
- [Formal paper — SSRN](https://ssrn.com/abstract=6533358) — preprint of April 2026. **Read with the correction below.** Its non-bypassability claim — that no path to state modification exists which does not pass through the gate — appears in neither its property-to-test appendix nor its observed-guarantees list nor its stated limits. A later self-audit reports that gap and the commit-boundary defect found in August 2026; that write-up supersedes the April claim and should be read alongside it.
