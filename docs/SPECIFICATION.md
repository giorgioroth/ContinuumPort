# ContinuumPort CP-Core v1.0 — Full Specification
...
**Why these exclusions matter:** Emotional and behavioral continuity:
...
- Valid JSON syntax (must parse without errors)
- Required fields present (`cp`, `intent`, `state`, `timestamp`)
...
- No PII detected (email, phone, address patterns)
- No authentication tokens
- No recursive structures (JSON depth ≤ 10)
- Semantic density within range (prevent padding attacks)
...
- Resource exhaustion prevention (container size ≤ 5MB)
...
**Explanation:** Only required fields — minimal but valid.
...
```json
{
  ...
  "hints": [
    "tone: friendly and encouraging" // Behavioral directive → INVALID
  ],
  "timestamp": "2025-12-21T10:00:00Z"
}
```
...
- Enforce length and size limits
- Detect security patterns

**Reference implementation:**  
`cp-core/reference/validator.py validate examples/minimal.json`
...
**Anti-pattern:** Platforms **MUST NOT**:
